...
//Dibuja las card de los productos
...
//Incorpora productos al carrito, si hay stock y los pushea al nuevo array guarda el mismo en el storage
function agregarCarrito(compra) {
  if (hayStock(catalogo, compra) > 0) {
    Swal.fire({
      title: compra.nombre,
      text: `Agregaste tu producto al carrito`,
      imageUrl: compra.imagen,
      imageWidth: 70,
      imageHeight: 70,
      imageAlt: compra.nombre,
    });
    carrito.push(compra);
...
//recorre el array y dibuja cada card
function dibujarCatalogo() {
  catalogo.forEach((element) => dibujar(element));
}

// Refiere a la api donde está guardado el catálogo
fetch("https://6377d2310992902a2511f740.mockapi.io/productos")
